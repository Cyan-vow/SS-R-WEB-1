...
cd /usr/local
git clone https://github.com/Cyan-vow/SS-R-
cd ./shadowsocksr
...
if [ -e /usr/local/bin/ssr ];then
    ...
    if [[ ${yn} == [yY] ]];then
        ...
        rm -rf /usr/local/shadowsocksr
        cd /usr/local
        git clone https://github.com/shangshanzhiyi/SS-R-
        if [[ $1 == develop ]];then
            cd ./shadowsocksr
            git checkout stack/dev
            rm -f ./mudb.json
...
if [[ -d /usr/local/SSR-Bash-Python ]];then
    if [[ $yn == [yY] ]];then
        rm -rf /usr/local/SSR-Bash-Python
        cd /usr/local
        git clone https://github.com/Cyan-vow/SS-R-.git
    fi
...
if [[ $1 == "develop" ]];then
	wget -q -N --no-check-certificate -O /usr/local/bin/ssr https://raw.githubusercontent.com/Cyan-vow/SS-R-WEB-1/master/ssr
...
if [[ $1 == develop ]];then
    if [[ -e /usr/local/SSR-Bash-Python/check.log ]];then
        cd /usr/local/SSR-Bash-Python
        read -n 1 -t 3 -p "你是否想要重新配置舰船巡检配置（注意，这将会清空你的操纵日志）[Y/N]" yn
        if [[ $yn == [Yy] ]];then
...
if [[ ${check} != "yes" ]] ;then
        echo "如果你执行 启动 提示找不到命令，请尝试退出并重新登录来解决"
fi
